...
import {
  useValidateBulkScheduled,
  useUploadBulkScheduled,
  useBulkScheduledBatch,
  useDownloadBulkScheduledTemplate,
  useDownloadBulkScheduledResult,
} from '../model/use-bulk-scheduled-payouts'
import { isSchedTerminal, type BulkScheduledValidateResponse } from '../types/bulk'
...
const TITLES: Record<Step, string> = {
  select: 'Bulk Scheduled Payout Upload',
  review: 'Review schedules',
  processing: 'Creating schedules',
  done: 'Bulk schedule result',
}

const DESCRIPTIONS: Record<Step, string> = {
  select: 'Upload a filled template to schedule one-time payouts.',
  review: 'Review the parsed schedules before confirming.',
  processing: 'Your schedules are being created.',
  done: 'Processing complete.',
}

interface BulkScheduledUploadModalProps {
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function BulkScheduledUploadModal({ open, onOpenChange }: BulkScheduledUploadModalProps) {
  const [step, setStep] = useState<Step>('select')
  const [file, setFile] = useState<File | null>(null)
  const [report, setReport] = useState<BulkScheduledValidateResponse | null>(null)
  const [batchId, setBatchId] = useState<string | null>(null)
  const [validationErrors, setValidationErrors] = useState<string[]>([])

  const templateMut = useDownloadBulkScheduledTemplate()
  const validateMut = useValidateBulkScheduled()
  const uploadMut = useUploadBulkScheduled()
  const resultMut = useDownloadBulkScheduledResult()
  const batchQuery = useBulkScheduledBatch(batchId, step === 'processing' || step === 'done')
  const batch = batchQuery.data ?? null

  const effectiveStep: Step =
    step === 'processing' && isSchedTerminal(batch?.status) ? 'done' : step
  ...
  // Balance is advisory for schedules — only valid + not-duplicate gate Confirm.
  const canConfirm = report?.valid === true && !report.duplicate
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // trigger_time can slip into the past between validate and confirm → 422.
      toast.error(getErrorMessage(err))
      setStep('select')
      setReport(null)
    }
      ...
      >
        ...
        {effectiveStep === 'review' && report && (
          <div className="flex min-w-0 flex-col gap-6">
            <div className="grid grid-cols-2 gap-3">
              <Stat label="Total schedules" value={String(report.total_rows)} />
              <Stat label="Total amount" value={formatINR(report.total_amount)} />
            </div>

            {report.wallets.length > 0 && (
              <div className="flex flex-col gap-2 rounded-md border-[0.4px] border-foreground/40 bg-secondary px-4 py-3">
                <span className="text-xs font-medium text-muted-foreground">Wallets</span>
                {report.wallets.map(w => {
                  const over = w.wallet_balance != null && w.scheduled_total > w.wallet_balance
                  return (
                    <div
                      key={w.wallet_code}
                      className="flex items-center justify-between gap-3 text-xs"
                    >
                      <span className="font-medium text-foreground">{w.wallet_code}</span>
                      <span className="flex items-center gap-2 text-muted-foreground">
                        scheduled {formatINR(w.scheduled_total)}
                        {w.wallet_balance != null && ` of ${formatINR(w.wallet_balance)}`}
                        {over && (
                          <span className="rounded-full bg-status-orange/10 px-2 py-0.5 font-medium text-status-orange">
                            exceeds balance
                          </span>
                        )}
                      </span>
            ...
            <div className="flex items-stretch gap-2">
              ...
              >
                {uploadMut.isPending
                  ? 'Submitting…'
                  : `Confirm ${report.total_rows} ${report.total_rows === 1 ? 'schedule' : 'schedules'}`}
              </button>
            </div>
          </div>
        )}
        ...
        {effectiveStep === 'processing' && (
          <BulkProcessingStep
            ...
            errorMessage="We couldn’t fetch the status. Your schedules may still be processing in the background — check the scheduled payouts list shortly."
            onClose={close}
          />
        )}
